...
format2_train_data['activity_log'] = format2_train_data['activity_log'].fillna('').apply(lambda x: '#' + '#'.join([s for s in x.split('#') if len(s.split(':')) > 1 and int(s.split(':')[0]) not in item_ids]) + '#')
...
format2_test_data['activity_log'] = format2_test_data['activity_log'].fillna('').apply(lambda x: '#' + '#'.join([s for s in x.split('#') if len(s.split(':')) > 1 and int(s.split(':')[0]) not in item_ids]) + '#')
